...
public class TextSizer : MonoBehaviour
{
    ...
    IEnumerator RefreshSize()
    {
        ...
        while (Time.time - startTime < RefreshTime)
        {
            UpdateTextSize();
            yield return null;
        }
    ...
    {
        if(text == null) return;
        // if ((text.text != lastText || lastSize != rt.rect.size || forceRefresh))
        bool isTextEqual = text.text == lastText;
        bool isSizeEqual = lastSize.y == rt.rect.size.y;
        // Debug.Log($"lastSize = {lastSize}, currentSize = {rt.rect.size}, text = {text.text}");
        // Debug.Log($"text equal = {isTextEqual}, size = {isSizeEqual}, force = {forceRefresh}, text = {text.text}");
        if(!isTextEqual)
        {
            StopAllCoroutines();
            StartCoroutine(RefreshSize());
    ...
    private bool IsSizeInTolerance()
    {
        float sizeTolerance = 10f;
        Debug.Log($"lastSize = {lastSize}, currentSize = {rt.rect.size}, text = {text.text}");
        bool isXInTolerance = Mathf.Abs(lastSize.x - rt.rect.size.x) < sizeTolerance;
        bool isYInTolerance = Mathf.Abs(lastSize.y - rt.rect.size.y) < sizeTolerance;
        return isXInTolerance && isYInTolerance;
    }

    private void UpdateTextSize()
    {
        // Debug.Log($"Updating Text Size!! {text.text} with textSize = {lastSize}");
        lastText = text.text;
        // Vector2 preferredSize = text.GetPreferredValues(MaxX, MaxY);
        Vector2 preferredSize = new Vector2(text.renderedWidth, text.renderedHeight);
